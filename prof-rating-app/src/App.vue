...
export default {
  ...
  methods: {
    ...
    editEntry: function(e) {
      axios
        .put("http://localhost:8080/prof" + e.index, {
          name: e.name,
          rating: e.rating
        })
        .then(response => {
    ...
    removeEntry: function(e) {
      axios.delete("http://localhost:8080/prof/" + this.listOfEntries[e.index]._id).then(response => {
        this.listOfEntries = response.data;
      });
    }
  },
  mounted() {
    axios.get("http://localhost:8080/prof").then(response => {
      this.listOfEntries = response.data;
      alert(this.listOfEntries);
    });
  }
};
</script>
